...
if (document.querySelector('.main-content h1')?.textContent.includes('Admin Dashboard')) {
  api.getAdminDashboardData().then(data => {
    ...
    function formatCurrency(value) {
      ...
      return !isNaN(num) ? `₹${num.toLocaleString('en-IN')}` : '₹0';
    }

    // Update Grants
    const grantsAllocated = formatCurrency(data.grants_allocated);
    document.querySelectorAll('.stat-value')[1].textContent = grantsAllocated;

    const grantsUtilized = formatCurrency(data.grants_utilized);
    const grantsRemaining = formatCurrency(data.grants_remaining);

    document.querySelector('.stat-details').innerHTML = `
      <div>Utilized: ${grantsUtilized}</div>
      <div>Remaining: ${grantsRemaining}</div>
      `;
